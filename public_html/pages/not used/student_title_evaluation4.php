<?php
              ...
              if($result){
                  ...
                  echo'
                      <script>
                            ...
                            document.getElementById("scopeID").value = ' . json_encode($scope) . ';
                      </script>
                    ';
              }
  ...
  function saveScope(){
      ...
      try{
          ...
          $sql = "UPDATE title_proposal SET scope = ? WHERE projectID = ? AND titleNum = ?";
          $stmt = $conn->prepare($sql);
          $result = $stmt->execute([$_SESSION["scope"], $_SESSION["projectID"], $_SESSION["titleNum"]]);
          ...
          if($result){
              ...
              echo '<script>
                    Swal.fire({
                         title: "Success",
                        text: "Scope of the Study Succesfuly Saved!",
                        icon: "success",
                        confirmButtonText: "OK"
                    }).then((result) => {
                        if (result.isConfirmed) {
                            window.location.href = "/edit_title_evaluation_scope";
                        }
                    });
                  </script>';
          }
        ...
        unset($_POST["save-button"]);
            ...
            echo '<script>
                Swal.fire({
                     title: "Error Saving Scope of the Study",
    ...
    <body>
        ...
        <?php include 'menu.php'; ?>
            ...
            <form method="POST" action="">
                ...
                <div class="team-info">
                    <textarea id="scopeID" name="scope" rows="8" style="height: 300px; font-size: 17px; font-weight: bold;" placeholder="Scope of the Study Paragraph..."></textarea>
                </div>
                
                <br>
                
                <div>
                    <button class="save-button" name="save-button" type="Submit">Save Scope of Study</button>
                </div>
