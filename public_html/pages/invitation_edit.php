<?php
    ...
    function getStatus(){
        ...
        try{
            ...
            $stmt->execute([$_SESSION["projectID"], $_SESSION["recepientID"], $_SESSION["acadYearID"]]);
            ...
            if ($result && !empty($result["filepath"])) {
                ...
                if (file_exists($filepath)) {
                    ...
                    if($status == "accepted"){
                        echo ' 
                            <script>
                                document.getElementById("acceptStatus").innerHTML = "Invitation Accepted &#x2705";
    ...
    function getTrackingNum(){
        ...
        try{
            ...
            if($result){
                ...
                echo '
                    <p class="document-tracking" id="trackingLabel">Tracking Number: '.$_SESSION["trackingNum"].'</p>
                ';
            }
    ...
    function updateLogs($category) {
        ...
        try {
            ...
            if ($userResult) {
                ...
                if($category == "upload"){
                    $description = "Student: " . $surname . ", " . $firstname . " updated the defense invitation";
                }
                
                else if($category == "comment"){
                    $description = "Student: " . $surname . ", " . $firstname . " updated the defense invitation and comments";
                }
                    ...
                    if($result){
                        ...
                        if($status == "started"){
                            ...
                            if($result){
                                ...
                                throw new Exception("Failed to update document tracking status of invitation to submitted");
                            }
                        }
                    ...
                    Swal.fire({
                        ...
                        confirmButtonText: "OK"
                            ...
                            if (result.isConfirmed) {
                                window.location.href = "/send_defense_invitation";
                            }
    ...
    function uploadFile($comment) {
        ...
        try {
            ...
            $requiredSessionVars = ["projectID", "recepientID", "acadYearID", "trackingNum"];
            ...
            $recipientDir = $targetDir . $_SESSION["recepientID"] . "-" . $_SESSION["acadYearID"];
            if (!is_dir($recipientDir) && !mkdir($recipientDir, 0777, true)) {
                throw new Exception("Failed to create recipient directory: $recipientDir");
            }
    
            $targetFilePath = $recipientDir . '/' . $fileName;
            ...
            $checkSql = "SELECT COUNT(*) FROM invitations WHERE projectID = ? AND facultyID = ? AND academicYearID = ?";
            $checkStmt = $conn->prepare($checkSql);
            $checkStmt->execute([
                $_SESSION["projectID"], 
                $_SESSION["recepientID"], 
                $_SESSION["acadYearID"]
            ]);
    
            // Set fileExists based on database presence
            ...
            if (!$fileExists) {
                ...
                $sql = "INSERT INTO invitations (projectID, facultyID, status, submit_date, academicYearID, filepath, comment, trackingNum) VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->execute([
                    $_SESSION["projectID"], 
                    $_SESSION["recepientID"], 
                ...
                if ($rowCount > 0) {
                    ...
                    if($result){
                        ...
                        if($result){
                            ...
                            $desc =  $section . " Group " . $groupNum . " sent a capstone defense invitation to you" ;
                            ...
                            $stmt = $conn->prepare($sql);
                            $result = $stmt->execute([$_SESSION["recepientID"], $desc, $date]);
                            ...
                            if(!$result){
                             ...
                             $sql = "SELECT * FROM users WHERE id = ?";
                             $stmt = $conn->prepare($sql);
                             $stmt->execute([$_SESSION["recepientID"]]);
                             $result = $stmt->fetch(PDO::FETCH_ASSOC);
                             ...
                             $faculty_name = "". $surname . ", " . $firstname . " " . $middlename . "";
                             ...
                             $action = "". $surname . ", " . $firstname . " " . $middlename . " sent a capstone defense invitation to: " . $faculty_name;
               ...
               $date = date("Y-m-d H:i:s");
                ...
                $sql = "UPDATE invitations SET filepath = ?, comment = ?, submit_date = ? WHERE projectID = ? AND facultyID = ? AND academicYearID = ?";
                $stmt = $conn->prepare($sql);
                $stmt->execute([
                    $targetFilePath, 
                    $comment, 
                ...
                if ($rowCount > 0) {
                     ...
                     $action = "". $surname . ", " . $firstname . " " . $middlename . " re-sent a capstone defense invitation to: " . $faculty_name;
    ...
    function updateComment($comment){
        ...
        try{
            ...
            $sql = "UPDATE invitations SET comment = ? WHERE projectID = ? AND facultyID = ? AND academicYearID = ?";
            $stmt = $conn->prepare($sql);
            $result = $stmt->execute([$comment, $_SESSION["projectID"], $_SESSION["recepientID"], $_SESSION["acadYearID"]]);
            ...
            if($result){
                ...
                throw new Exception("Failed creating invitation");
           }
        }
...
<html lang="en">
    <head>
        ...
        <link rel="stylesheet" href="pages/invitation.css">
    ...
    <body>
        ...
        <!--<div class="semester-container">
            ...
            <p class="due-date-info">Due Date: November 30, 2023</p>
        </div>-->
            ...
            <div class="horizontal-container">
                <div class="left-side">
                    ...
                    <div class="comments-section">
                        ...
                        <textarea id="comments" name="inviteComment" rows="4" placeholder="Optional Comments..."><?php getComment();?></textarea>
                    </div>
                </div>
                ...
                <div class="right-side">
                    ...
                    <div class="panelist-section">
                        <br>
                          <label for="file-upload">Panelist</label>
                        <div class="panelist-info">
                            
                            <i class="fas fa-user-circle"></i>
                            <p id="facultyName2" class="name-color"><?php echo htmlspecialchars($_SESSION["facultyName"]); ?></p>
                        </div>
                        ...
                        <div class="panelist-info-buttons">
                            ...
                            <img id="loading-gif" src="pages/images/loading.gif" alt="Uploading File..." style="display: none; width: 100px; height: 100px; margin-left: 70px; margin-top: 70px; position: fixed;">
        ...
        <?php 
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (isset($_POST['ajax']) && $_POST['ajax'] == 'true') {
                     if (isset($_POST["facultyID"]) && isset($_POST["facultyName"]) && isset($_POST["facultyRole"])) {
                            
                            $_SESSION["recepientID"] = $_POST["facultyID"];
                            $_SESSION["facultyName"] = $_POST["facultyName"];
                            $_SESSION["facultyRole"] = $_POST["facultyRole"];
                            
                            echo '
                                <script>
                                    document.getElementById("facultyName").innerHTML = "'.$_SESSION["facultyName"].'";
                                    document.getElementById("facultyName2").innerHTML = "'.$_SESSION["facultyName"].'";
                                    document.getElementById("facultyRole").innerHTML = "'.$_SESSION["facultyRole"].'";
                                </script>
                            ';
                    }
                 }
                    ...
                    if (isset($_FILES['uploaded_file']) && $_FILES['uploaded_file']['error'] === UPLOAD_ERR_OK) {
                        uploadFile($_POST["inviteComment"]);
                    }
                        ...
                        if($fileExists == false){
                            ...
                            updateComment($_POST["inviteComment"]);
                        }
                    }
                }
            }
