...
<?php 
    ...
    function addPanelist(){
        ...
        try{
            ...
            $panelistID = isset($_POST["facultyBox"]) ? htmlspecialchars($_POST["facultyBox"]) : '';
            ...
            if($panelistID != "None"){
                ...
                if($countResult){
                    ...
                    if(($current_count + 1) <= $current_limit){
                        ...
                        $stmt = $conn->prepare($sql);
                        $result = $stmt->execute([$newCount, $panelistID]);
                        
                        if($result){
                        
                            $sql = "INSERT INTO panelists (panelistID, projectID, level, academicYearID) VALUES (?, ?, ?, ?)";
                            $stmt = $conn->prepare($sql);
                            $result = $stmt->execute([$panelistID, $projectID, 2, $acadYear]);
                            ...
                            if($result){
                                ...
                                $desc = $coord_name . " added you as a CAPSTONE PANELIST CHAIRMAN for a capstone group in " . $section;
                                ...
                                $stmt = $conn->prepare($sql);
                                $result = $stmt->execute([$panelistID, $desc, $date]);
                                ...
                                if($result){
                                     ...
                                     $chairman_name = "". $result["surname"] . ", " . $result["firstname"]  . " " . $result["middlename"] ;
                                     ...
                                     $action = "". $surname . ", " . $firstname . " " . $middlename . " added " . $chairman_name . " as the chairman panelist of Group " . $groupNum . " in " . $section;
                                    ...
                                    unset($_POST["add-button"]);
                                     ...
                                     echo '<script>
                                        Swal.fire({
                                             title: "Success",
                                            text: "Capstone Chairman Panelist successfully added!",
                                            icon: "success",
                                            confirmButtonText: "OK"
                                        }).then((result) => {
                                            if (result.isConfirmed) {
                                                window.location.href = "/add_chairman";
                                            }
                                ...
                                throw new Exception("Error inserting new chairman panelist");
                            }
                        }
                        
                        else{
                            throw new Exception("Failed to update panelist count of selected faculty");
                        }
                    }
                    
                    else if(($current_count + 1) > $current_limit){
                        throw new Exception("Panelist Limit of Faculty: " . ucwords($name) . " Has Been Reached");
                    }
                }
            }
            
            else{
                 throw new Exception("No Panelists Currently Available");
            }
            ...
            $conn->rollBack();
            error_log("Error inserting panelist: " . $e->getMessage());
            
            unset($_POST["add-button"]);
            
            echo '<script>
                Swal.fire({
                     title: "Error Adding Chairman Panelist",
                    text: "Chairman Panelist could not be added: '.addslashes($e->getMessage()).'",
                    icon: "error",
                    confirmButtonText: "OK"
                }).then((result) => {
                    if (result.isConfirmed) {
...
<head>
    ...
    <title>Add Chairman Panelist</title>
...
<body>
    ...
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@10"></script>
        ...
        <div class="header-group">
            <h2>Adding Chairman Panelist For:</h2>
            ...
            <div class="optionDiv">
                <div class="right-section">
                    ...
                    <div class="button-group">
                        <button class="form-button" type="submit" name="add-button">Add Chairman</button>
                    </div>
                </div>
            </div> 
        </form>
