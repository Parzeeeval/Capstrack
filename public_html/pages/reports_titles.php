<?php
   ...
   function generateReport() {
        ...
        try {
            ...
            header('Content-Disposition: attachment; filename="Capstone_Titles_Tags_Report ' . date('F_d_Y') . '.xlsx"');
            ...
            $sheet->setTitle("Capstone Titles"); // Limit sheet title to 30 characters
            ...
            $sql = "SELECT * FROM capstone_projects WHERE projectID <> '0' AND academicYearID = ?";
            $stmt = $conn->prepare($sql);
            ...
            foreach ($projects as $project){
                ...
                $sheet->setCellValue("A$row", htmlspecialchars($section_name));
                $sheet->setCellValue("B$row", htmlspecialchars($project["groupNum"]));
                $sheet->setCellValue("C$row", htmlspecialchars($project["title"]));
            ...
            $sheet->getStyle('A2:D' . $row)->applyFromArray($dataAlignment);
            ...
            $sheet->getStyle('A1:D' . $row)->applyFromArray([
             ...
             // Create a new sheet for this user
            $sheet = $spreadsheet->createSheet();
            $sheet->setTitle("Title Tags with Count"); // Limit sheet title to 30 characters
            ...
            foreach ($tags as $tag) {
                ...
                // Get the count of titles associated with this tag
                $sql = "SELECT COUNT(*) as count FROM title_tags WHERE tag = ?";
                $stmt = $conn->prepare($sql);
                $stmt->execute([$tagname]);
                $count = $stmt->fetch(PDO::FETCH_ASSOC)["count"];
                
                $displayName = ucwords(str_replace('_', ' ', $tagname));
           ...
           // Center align the content in A2:C$row
            ...
            $sheet->getStyle('A2:C' . $row)->applyFromArray($dataAlignment);
        ...
        <div id="content2">
            <div class="profile-tabs">
                <div class="tabs">
                    ...
                    <button class="active" onclick="window.location.href='/reports_titles';">Capstone Titles</button>
       ...
       <!--<h2>Accounts</h2>
        ...
        <div class="accounts-body">
            <div class="left-section">
              <p class="divider-label">Title Tags & Rankings</p>
              ...
              <div class="tags-container">
                ...
                <?php
                    ...
                    // Prepare the JavaScript array to hold the data
                    echo '<script>';
                    echo 'const tagsData = [';
                    ...
                    foreach ($tags as $index => $tag) {
                        ...
                        // Transform tag name for display
                        $displayName = ucwords(str_replace('_', ' ', $tagname));
                    
                        // Add tag data to the JavaScript array
                        echo "{ name: '" . $displayName . "', count: " . $count . " }";
                ...
                <script>
                    ...
                    const sortedTags = tagsData.sort((a, b) => b.count - a.count);
                    ...
                    groupedTags.push(currentGroup); // Push the last group
                    
                    // Function to assign colors based on group index
                    const assignColor = (groupIndex) => {
                        if (groupIndex === 0) return 'gold';
                        if (groupIndex === 1) return 'silver';
                        if (groupIndex === 2) return 'bronze';
                        return 'white';
                    };
                    ...
                    // Populate container with sorted and color-coded cards
                    groupedTags.forEach((group, groupIndex) => {
                        const colorClass = assignColor(groupIndex);
                        group.forEach((tag) => {
                            // Tags with 0 count are always white
                            const cardClass = tag.count === 0 ? 'white' : colorClass;
                    
                            const card = document.createElement('div');
                            card.className = `tag-card ${cardClass}`;
                            card.innerHTML = `
                                <div class="card-content">
                                    <div class="tag-name">${tag.name}</div>
                                    <div class="number">${tag.count}</div>
                                </div>
                            `;
            ...
            <div class="right-section">
                ...
                <p class="divider-label">Capstone Titles</p>
                <div class="divider"></div>
        ...
        <div class="accounts-footer">
            <form action="" method="POST">
                <button type="submit" name="report-btn" class="btn-update">Generate Titles Report</button>
            </form>
        </div>
